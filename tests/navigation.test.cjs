...
test('screen planning pages stay within their eight-page list through buttons and keyboard routes', () => {
  const app = boot();
  const ids = ['rist-cover', 'rist-login', 'rist-upload', 'rist-processing', 'rist-error', 'rist-review-preparation', 'rist-review-detail', 'rist-review-detail-55'];
  const markers = [0, 2, 4, 3, 3, 11, 8, 8];
  assert.equal(app.get('page-breadcrumb').textContent, '화면 기획서');
  app.get('previous-page').click(); app.key('ArrowLeft'); app.iframe('previous');
  expectPage(app, 'rist-cover', '01/08');
  ids.forEach((id, index) => {
    expectPage(app, id, String(index + 1).padStart(2, '0') + '/08');
  ...
  expectPage(app, 'rist-review-detail-55', '08/08');
  assert.equal(app.get('description-page-label').textContent, '현재 화면 · 08');
  for (let i = ids.length - 2; i >= 0; i--) {
    app.key('ArrowLeft');
    expectPage(app, ids[i], String(i + 1).padStart(2, '0') + '/08');
  }
});
...
test('login description links cross from screen planning to the policy list without changing IDs', () => {
  ...
  expectPage(app, 'rist-login', '02/08');
...
test('review stage tabs and footer page links route between registered pages without entering groups', () => {
  const app = boot({ hash: '#rist-review-preparation' });
  app.pageLink('rist-review-detail');
  expectPage(app, 'rist-review-detail', '07/08');
  assert.equal(app.location.hash, '#rist-review-detail');
  app.pageLink('rist-review-detail-55');
  expectPage(app, 'rist-review-detail-55', '08/08');
  app.pageLink('rist-review-detail');
  app.pageLink('rist');
  app.pageLink('missing-page');
  expectPage(app, 'rist-review-detail', '07/08');
  app.pageLink('rist-review-preparation');
  expectPage(app, 'rist-review-preparation', '06/08');
});
...
test('extraction rule pages share a separate index-first scope and preserve document anchor links', () => {
  ...
  expectPage(app, 'extraction-rules-index', '01/02');
  assert.equal(app.get('page-breadcrumb').textContent, '추출 규칙');
  app.key('ArrowLeft');
  expectPage(app, 'extraction-rules-index', '01/02');
  app.pageLink('extraction-rule-csv-001');
  expectPage(app, 'extraction-rule-csv-001', '02/02');
  app.key('ArrowRight'); app.iframe('next'); app.get('next-page').click();
  expectPage(app, 'extraction-rule-csv-001', '02/02');
