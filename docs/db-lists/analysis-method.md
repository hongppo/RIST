...
| `description` | 설명·적용 조건 | 파일 유형과 관계없이 분석물질에 대응하는 공통 분석법 매칭 적용. |
...
파일 유형과 관계없이 분석물질 필드에 추출되거나 입력된 데이터를 기준으로 [분석법 매칭](<../matching-rules/analysis-method.md>)을 적용한다. 대응이 없거나 불명확하면 `NULL`로 두고 검토 화면에서 보완한다.
